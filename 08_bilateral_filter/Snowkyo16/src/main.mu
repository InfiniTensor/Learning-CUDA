...
// 主程序 —— 版本调度器（Moore Threads S5000 平台）
int main(int argc, char* argv[]) {
    if (argc < 4) {
        cerr << "用法: " << argv[0]
             << " <输入图像> <参数文件> <输出目录> [版本]" << endl;
        cerr << "版本: v0 / v6 / all（默认 all）" << endl;
        return 1;
    }
    ...
    // 加载输入
    cout << "========================================" << endl;
    cout << "  双边滤波 — 运行模式: " << mode << endl;
    cout << "  平台: Moore Threads S5000 (MUSA)" << endl;
    cout << "========================================" << endl;
    ...
    int gpu_runs = 10;  // GPU 版本：1 次预热 + 10 次计时取平均

    if (mode == "v0" || mode == "all")
        results.push_back(run_version("v0_cpu", input, params,
                                      bilateral_filter_cpu_v0, image_dir, basename, ext));

    if (mode == "v6" || mode == "all") {
        cout << endl << "  [v6_moore 初始化]" << endl;
        bilateral_filter_gpu_v6_init(input, params);
        results.push_back(run_version("v6_moore", input, params,
                                      bilateral_filter_gpu_v6, image_dir, basename, ext, gpu_runs));
        bilateral_filter_gpu_v6_cleanup();
    }
