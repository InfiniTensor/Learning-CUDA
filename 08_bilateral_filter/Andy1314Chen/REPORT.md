...
1. **全局内存访问量大**：邻域像素被相邻线程重复读取，带宽浪费严重。x
...
| **Driver** | 580.00 |
| **JetPack** | R38.2.1 |
| **OpenCV** | 4.x（with CUDA modules, cv::cuda::bilateralFilter） |

**关键架构特性**：Jetson AGX Thor 采用**统一内存架构（Unified Memory）**，CPU 和 GPU 共享物理内存，无 PCIe 总线。`cudaMemcpy` 本质上是内存拷贝而非 DMA 传输，开销远小于独显平台。这从根本上改变了性能瓶颈分布。OpenCV 已编译 CUDA 模块，`cv::cuda::bilateralFilter` 可作为 GPU 级基准。
...
| Implementation | Avg (ms) | Min (ms) | Throughput (MP/s) | MAE | PSNR (dB) | vs OCV CPU | vs OCV CUDA |
|----------------|:--------:|:--------:|:-----------------:|:---:|:---------:|:----------:|:-----------:|
| **SEP_FP16** | **4.51** | **2.93** | **2838** | **0.46** | **48.39** | **74.8x** | **4.23x** |
| SEPARABLE | 3.22 | 2.97 | 2577 | 0.45 | 48.49 | 101.7x | 8.11x |
| FUSED | 4.46 | 3.92 | 1858 | 0.45 | 48.49 | 63.6x | 4.38x |
| TEMPLATE | 5.45 | 5.43 | 1522 | 0.60 | 48.28 | 15.0x | 2.23x |
| ADAPTIVE | 7.25 | 6.07 | 1144 | 0.40 | 49.42 | 43.3x | 3.20x |
| STANDARD | 9.43 | 9.27 | 880 | 0.48 | 48.61 | 34.8x | 2.51x |
| **OpenCV CUDA** | 12.14 | 10.98 | 683 | 0.00 | 999.99 | — | 1.00x |
| OpenCV CPU | 81.95 | 80.94 | 101 | — | — | 1.0x | — |

### 10.4 4K Grayscale 性能结果 (3840x2160x1)

| Implementation | Avg (ms) | Min (ms) | Throughput (MP/s) | MAE | PSNR (dB) | vs OCV CPU | vs OCV CUDA |
|----------------|:--------:|:--------:|:-----------------:|:---:|:---------:|:----------:|:-----------:|
| **SEP_FP16** | **1.40** | **1.28** | **5944** | **0.12** | **57.00** | **36.9x** | **5.62x** |
| SEPARABLE | 1.48 | 1.39 | 5622 | 0.15 | 56.18 | 34.8x | 5.38x |
| FUSED | 1.80 | 1.60 | 4615 | 0.15 | 56.18 | 28.6x | 4.47x |
| TEMPLATE | 3.51 | 3.46 | 2365 | 0.61 | 50.23 | 24.0x | 2.26x |
| STANDARD | 4.37 | 4.33 | 1900 | 0.61 | 50.23 | 11.9x | 1.78x |
| ADAPTIVE | 5.85 | 5.76 | 1419 | 0.61 | 50.23 | 8.8x | 1.34x |
| **OpenCV CUDA** | 7.92 | 7.57 | 1047 | 0.00 | 999.99 | — | 1.00x |
| OpenCV CPU | 84.23 | 51.01 | 98 | — | — | 1.0x | — |
...
| Implementation | Avg (ms) | Min (ms) | Throughput (MP/s) | MAE | PSNR (dB) | vs OCV CPU | vs OCV CUDA |
|----------------|:--------:|:--------:|:-----------------:|:---:|:---------:|:----------:|:-----------:|
| **SEP_FP16** | **0.79** | **0.78** | **2618** | **0.46** | **48.36** | **47.3x** | **5.84x** |
| SEPARABLE | 1.02 | 0.77 | 2025 | 0.45 | 48.46 | 36.6x | 4.46x |
| FUSED | 1.04 | 1.03 | 2002 | 0.45 | 48.46 | 25.9x | 3.44x |
| TEMPLATE | 2.60 | 1.63 | 798 | 0.61 | 48.21 | 14.6x | 1.82x |
| STANDARD | 2.50 | 2.38 | 831 | 0.48 | 48.58 | 12.0x | 1.75x |
| ADAPTIVE | 2.15 | 1.57 | 965 | 0.41 | 49.34 | 14.5x | 1.64x |
| **OpenCV CUDA** | 4.93 | 3.03 | 420 | 0.00 | 999.99 | — | 1.00x |
| OpenCV CPU | 58.35 | 32.33 | 36 | — | — | 1.0x | — |

#### 1080p Grayscale

| Implementation | Avg (ms) | Min (ms) | Throughput (MP/s) | MAE | PSNR (dB) | vs OCV CPU | vs OCV CUDA |
|----------------|:--------:|:--------:|:-----------------:|:---:|:---------:|:----------:|:-----------:|
| **SEP_FP16** | **0.36** | **0.35** | **5790** | **0.12** | **56.85** | **59.5x** | **10.90x** |
| SEPARABLE | 1.25 | 0.36 | 1665 | 0.15 | 56.06 | 17.1x | 2.40x |
| FUSED | 1.85 | 0.45 | 1119 | 0.15 | 56.06 | 12.0x | 1.65x |
| TEMPLATE | 1.09 | 0.91 | 1904 | 0.61 | 50.18 | 23.3x | 2.89x |
| STANDARD | 1.51 | 1.11 | 1369 | 0.61 | 50.18 | 12.0x | 2.67x |
| ADAPTIVE | 1.71 | 1.48 | 1211 | 0.61 | 50.18 | 14.8x | 1.59x |
| **OpenCV CUDA** | 3.91 | 2.13 | 531 | 0.00 | 999.99 | — | 1.00x |
| OpenCV CPU | 38.10 | 20.88 | 54 | — | — | 1.0x | — |
...
### 12.2.1 4K RGB vs OpenCV CUDA（跨平台）

| Platform | Best CUDA Min (ms) | OCV CUDA Min (ms) | vs OCV CUDA |
|----------|:------------------:|:------------------:|:-----------:|
| RTX 4060 (SEPARABLE) | 5.41 | 11.36 | **2.10x** |
| **Thor (SEP_FP16)** | **2.93** | **10.98** | **3.74x** |
...
| vs OpenCV CPU | 显著加速 | **9~101x** | **7~20x** |
| vs OpenCV CUDA | 超越 | **1.34~10.9x** | **1.68~4.43x** |

**峰值性能**：SEPARABLE_FP16 (MODE=3) 在 Thor 上达到 **4K RGB 2.93ms (2838 MP/s)**，**4K Gray 1.28ms (5944 MP/s)**。SM throughput 75-81%，occupancy 97%，接近硬件极限。**vs OpenCV CUDA: 4K RGB 3.74x, 4K Gray 5.62x, 1080p Gray 10.9x**。
...
| vs OpenCV CUDA | **2.10x** (4K RGB) | **3.74x** (4K RGB), **10.9x** (1080p Gray) |
