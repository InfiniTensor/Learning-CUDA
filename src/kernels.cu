...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
  ...
  // Fallback CPU implementation matching torch.nn.functional.scaled_dot_product_attention
  // Layouts:
  //   Q: [B, T, QH, D]
  //   K,V: [B, S, KVH, D]
  //   O: [B, T, QH, D]
  ...
  // Map each query head to a kv head (GQA). Works for MHA (kv_heads == query_heads) too.
  auto kv_for_q = [=](int qh) {
    // Integer proportional mapping; assumes query_heads >= kv_heads
    return (kv_heads == 1) ? 0 : (qh * kv_heads) / query_heads;
  };
    ...
    for (int t = 0; t < target_seq_len; ++t) {
      for (int qh = 0; qh < query_heads; ++qh) {
        ...
        if (max_score == -std::numeric_limits<float>::infinity()) {
          max_score = neg_inf;  // all positions masked; keep weights zero
        }

        // Pass 2: compute softmax weights and weighted value sum
