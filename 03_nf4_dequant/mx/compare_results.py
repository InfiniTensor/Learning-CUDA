...
def compare_all():
    ...
    for mx_file in mx_files:
        ...
        if orig_file.exists():
            orig_tensor = torch.from_numpy(np.fromfile(orig_file, dtype=np.float16))
    ...
    with open(md_file, "w", encoding="utf-8") as f:
        ...
        for _, row in df.iterrows():
            f.write(
                f"| {row['shape']} | {row['blocksize']} | {row['bnb_time_ms']:.4f} | {row['mx_time_ms']:.4f} | {row['speedup']:.2f} | {row['mae']:.8f} | {row['mse']:.8f} | {row['max_diff']:.8f} | {row['bnb_error']:.8f} |\n"
            )
