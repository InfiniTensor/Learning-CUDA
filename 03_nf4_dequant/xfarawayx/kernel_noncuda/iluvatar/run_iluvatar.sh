...
KERNEL_DIR="${PROJ_DIR}/kernel_noncuda/iluvatar"
...
ILUVATAR_OUTPUT="${DATA_DIR}/iluvatar_output_${TAG}_${COMPUTE_TYPE}.bin"

build_kernel() {
    echo "[build] 使用编译器: ${ILCC_BIN}"
    make -C "${KERNEL_DIR}" clean >/dev/null
    make -C "${KERNEL_DIR}" ILCC="${ILCC_BIN}" -j"$(nproc)"
}
...
run_kernel() {
    ...
    "${KERNEL_DIR}/nf4_dequant_iluvatar" \
        "${WEIGHT_FILE}" "${ILUVATAR_OUTPUT}" "${COMPUTE_TYPE}" "${WARMUP}" "${REPEATS}"
}
