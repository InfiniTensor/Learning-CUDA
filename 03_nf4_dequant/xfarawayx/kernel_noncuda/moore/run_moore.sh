...
KERNEL_DIR="${PROJ_DIR}/kernel_noncuda/moore"
...
MOORE_OUTPUT="${DATA_DIR}/moore_output_${TAG}_${COMPUTE_TYPE}.bin"

build_kernel() {
    echo "[build] 使用编译器: ${MCC_BIN}"
    make -C "${KERNEL_DIR}" clean >/dev/null
    make -C "${KERNEL_DIR}" MCC="${MCC_BIN}" -j"$(nproc)"
}
...
run_kernel() {
    ...
    "${KERNEL_DIR}/nf4_dequant_musa" \
        "${WEIGHT_FILE}" "${MOORE_OUTPUT}" "${COMPUTE_TYPE}" "${WARMUP}" "${REPEATS}"
}
