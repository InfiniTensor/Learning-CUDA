...
// 预先先对NF4_LUT_HALF进行处理，转化为__half
...
// 利用共享内存存储scale，但当几个线程在进行存储时其他的线程并没有做其他的处理，时间并没有缩短
...
// 将NF4_LUT由常量内存放到共享内存中来
__global__ void nf4_dequant_v7(const uint8_t *__restrict__ packed,
                               const uint8_t *__restrict__ absmax_q,
                               const __half *__restrict__ absmax2, float offset,
                               int64_t total_elements, int blocksize,
                               int group_size, __half *__restrict__ output) {
  ...
  if(threadIdx.x<16){
    s_nf4[threadIdx.x]=NF4_LUT_HALF[threadIdx.x];
  }
  if (byte_idx >= total_bytes)
    return;
  uint32_t pack4 = ((const uint32_t *)packed)[tid];
  ...
  __half scale =
      __hadd(__hmul(CODE2_LUT[absmax_q[block_idx]], absmax2[group_idx]),
             __float2half(offset));
  __half h[8];
  h[0] = __hmul(s_nf4[(b0) >> 4], scale);
  h[1] = __hmul(s_nf4[(b0) & 0xF], scale);
  h[2] = __hmul(s_nf4[(b1) >> 4], scale);
  h[3] = __hmul(s_nf4[(b1) & 0xF], scale);
  h[4] = __hmul(s_nf4[(b2) >> 4], scale);
  h[5] = __hmul(s_nf4[(b2) & 0xF], scale);
  h[6] = __hmul(s_nf4[(b3) >> 4], scale);
  h[7] = __hmul(s_nf4[(b3) & 0xF], scale);
...
int main(int argc, char **argv) {
  ...
  int blocks = (total_bytes / 4 + threads - 1) / threads;
    ...
    int max_scale = (threads * 2 + blocksize - 1) / blocksize;

size_t smem = max_scale * sizeof(__half);
  printf("\n 内核配置:\n");
  printf("  blocks: %d\n", blocks);
  ...
  printf("\n 预热 (5次)...\n");
  for (int i = 0; i < 5; i++) {
    nf4_dequant_v7<<<blocks, threads>>>(d_packed, d_absmax_q, d_absmax2, offset,
                                        total_elements, blocksize, 256,
                                        d_output);
    // nf4_dequant_v5<<<blocks, threads>>>(d_packed, d_absmax_q, d_absmax2,d_code2, offset,
    //                                     total_elements, blocksize, 256,
    //                                     d_output);
    // nf4_dequant_v1<<<blocks, threads>>>(d_packed, d_absmax_q, d_absmax2,d_code2, offset,
    //                                     total_elements, blocksize, 256,
    //                                     reinterpret_cast<__half2*>(d_output));

    //  nf4_dequant_v4<<<blocks, threads, smem>>>(d_packed, d_absmax_q, d_absmax2, d_code2,
    //                                             offset, total_elements, blocksize, 256,
    //                                           reinterpret_cast<__half2*>(d_output));
