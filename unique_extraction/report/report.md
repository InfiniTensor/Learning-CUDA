...
- 测试数据： 500,000,000 个随机生成的 30(4.5GB)/64(9.5GB) 位特征量
...
学了下 openmp ， 利用 openmp 进行文件的并行读写优化， 充分利用我的 22 多线程， 拒绝 单核有难， 21 核围观， 直接上成果

### 输入优化

这里改变了文件的结构， 从单个大文件拆分成多个小文件进行并行读取数据， 测试文件格式为 $uint64\_\$\{elem_size\}\_\$\{file_id\}.in$ 如 $uint64\_16777216\_0.in$， 这样做一方面可以通过正则运算符对文件名进行元素数量的计算提前申请空间， 一方面为了并行读入所设计。

优化结果：
![](./picture/优化输入.png)

优化非常可观， 读入时间从一分半变为了 12 秒。

### 输出优化

对输出采用相同的优化， 考虑将去重结果都算出来后并行写出， 不过如果另申请一个大内存数组， 内存有点捉襟见肘了， 这里考虑复用 data 即输入时的数组， 因最后一步 标记-扫描-写回 过程中， 这一块的数据在 标记 过程中复制给 GPU 使命就完成了， 在 写回 阶段可以安心覆盖， 且输出数一定比输入的少， 所以不用害怕 越界覆盖。

优化结果：
![](./picture/优化输出.png)

效果依旧显著， 从 一分钟缩减到 10 秒钟。

### 结语

通过输入输出的优化， 时间从 4分半， 缩减到 两分十几秒， 差不多 2X 的加速比， 可喜可贺可喜可贺。

至于蒟蒻什么是否能写一般归并替换双调排序， 还是等蒟蒻再练练吧

> 二编时间 2025.10.15 22:44